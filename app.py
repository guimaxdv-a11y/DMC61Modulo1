...
# ============================================
    ...
    init_state("e1_concepto", "")
    init_state("e1_tipo", "Ingreso")
    init_state("e1_valor", 0.0)

    st.markdown("### Registrar nuevo movimiento")
    with st.form("form_ej1"):
        c1, c2, c3 = st.columns(3)
        with c1:
            concepto = st.text_input("Concepto", value=st.session_state["e1_concepto"], placeholder="Ej: Consulta médica")
        with c2:
            tipo = st.selectbox("Tipo", ["Ingreso", "Gasto"], index=["Ingreso", "Gasto"].index(st.session_state["e1_tipo"]))
        with c3:
            valor = st.number_input("Valor (S/.)", min_value=0.0, step=0.01, format="%.2f", value=st.session_state["e1_valor"])

        c_btn1, c_btn2 = st.columns(2)
        with c_btn1:
            agregar = st.form_submit_button("➕ Agregar movimiento", use_container_width=True)
        with c_btn2:
...
# ============================================
    ...
    init_state("e2_paciente", "")
    init_state("e2_categoria", "Hematología")
    init_state("e2_examen", "Hemoglobina")
    init_state("e2_resultado", 0.0)
    init_state("e2_referencia", 0.0)
    ...
    with st.form("form_ej2"):
        c1, c2 = st.columns(2)
        with c1:
            paciente = st.text_input("👤 Paciente", value=st.session_state["e2_paciente"], placeholder="Ej: Juan Pérez")
            categorias_lista = list(examenes.keys())
            categoria = st.selectbox("📋 Categoría", categorias_lista, index=categorias_lista.index(st.session_state["e2_categoria"]))
        with c2:
            examen_opts = examenes[categoria]["items"]
            current_examen = st.session_state["e2_examen"] if st.session_state["e2_examen"] in examen_opts else examen_opts[0]
            examen = st.selectbox("🔬 Examen", examen_opts, index=examen_opts.index(current_examen))
            unidad = examenes[categoria]["unidades"][examen]
            st.text_input("📏 Unidad", value=unidad, disabled=True)

        c3, c4 = st.columns(2)
        with c3:
            resultado = st.number_input("📊 Resultado", min_value=0.0, step=0.01, format="%.2f", value=st.session_state["e2_resultado"])
        with c4:
            referencia = st.number_input("📐 Valor de referencia", min_value=0.0, step=0.01, format="%.2f", value=st.session_state["e2_referencia"])

        c_btn1, c_btn2 = st.columns(2)
        with c_btn1:
            agregar = st.form_submit_button("➕ Agregar examen", use_container_width=True)
        with c_btn2:
            limpiar = st.form_submit_button("🧹 Limpiar campos", use_container_width=True)
    ...
    if agregar:
        if paciente.strip() == "" or resultado <= 0:
            ...
            estado = "Normal" if referencia > 0 and resultado <= referencia else "Alterado"
            ...
            st.session_state["e2_paciente"] = ""
            st.session_state["e2_resultado"] = 0.0
            st.session_state["e2_referencia"] = 0.0
            st.rerun()

    if limpiar:
        st.session_state["e2_paciente"] = ""
        st.session_state["e2_categoria"] = "Hematología"
        st.session_state["e2_examen"] = "Hemoglobina"
        st.session_state["e2_resultado"] = 0.0
        st.session_state["e2_referencia"] = 0.0
        st.rerun()
...
# ============================================
    ...
    init_state("e3_peso", 70.0)
    init_state("e3_altura", 1.75)

    st.markdown("### Ingresa tus datos")
    with st.form("form_ej3"):
        c1, c2 = st.columns(2)
        with c1:
            peso = st.number_input("⚖️ Peso (kg)", min_value=1.0, max_value=300.0, step=0.5, format="%.1f", value=st.session_state["e3_peso"])
        with c2:
            altura = st.number_input("📏 Altura (m)", min_value=0.50, max_value=2.50, step=0.01, format="%.2f", value=st.session_state["e3_altura"])
        ...
        with c_btn2:
            limpiar = st.form_submit_button("🧹 Limpiar campos", use_container_width=True)
        with c_btn3:
            reiniciar = st.form_submit_button("🗑️ Reiniciar historial", use_container_width=True)
    ...
    if calcular:
        if calcular_imc is None or calcular_superficie_corporal is None:
            ...
            try:
                ...
                st.session_state["e3_peso"] = 70.0
                st.session_state["e3_altura"] = 1.75
                st.rerun()
...
# ============================================
    ...
    init_state("edit_index", None)
    init_state("edit_nombre", "")
    init_state("edit_peso", 70.0)
    init_state("edit_altura", 1.75)
    ...
    if Paciente is None:
        ...
        with st.form("form_ej4"):
            c1, c2, c3 = st.columns(3)
            with c1:
                nombre = st.text_input("👤 Nombre", value=st.session_state["edit_nombre"], placeholder="Ej: Juan Pérez")
            with c2:
                peso = st.number_input("⚖️ Peso (kg)", min_value=1.0, max_value=300.0, step=0.5, format="%.1f", value=st.session_state["edit_peso"])
            with c3:
                altura = st.number_input("📏 Altura (m)", min_value=0.50, max_value=2.50, step=0.01, format="%.2f", value=st.session_state["edit_altura"])

            c_btn1, c_btn2, c_btn3, c_btn4 = st.columns(4)
            with c_btn1:
                crear = st.form_submit_button("➕ Crear", use_container_width=True)
            with c_btn2:
        ...
        if crear:
            if not nombre.strip():
                ...
                if ok:
                    ...
                    st.session_state["edit_peso"] = 70.0
                    st.session_state["edit_altura"] = 1.75
        ...
        if not st.session_state["pacientes"]:
            ...
            if st.button("▶️ Ejecutar", use_container_width=True):
                if accion == "✏️ Editar":
                    ...
                    st.session_state["edit_nombre"] = paciente.nombre
                    st.session_state["edit_peso"] = paciente.peso_kg
                    st.session_state["edit_altura"] = paciente.altura_m
                    st.session_state["edit_index"] = id_sel
                    ...
                    if ok:
                        st.success(msg)
                        if st.session_state["edit_index"] == id_sel:
                            st.session_state["edit_index"] = None
                            st.session_state["edit_nombre"] = ""
                            st.session_state["edit_peso"] = 70.0
                            st.session_state["edit_altura"] = 1.75
            ...
            if st.session_state["edit_index"] is not None and st.session_state["edit_index"] < len(st.session_state["pacientes"]):
                p = st.session_state["pacientes"][st.session_state["edit_index"]]
                st.info(f"✏️ Editando: **{p.nombre}** (ID {st.session_state['edit_index']})")
